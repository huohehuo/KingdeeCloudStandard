...
public class ReView4DBBoxAdapter extends BaseAdapter implements View.OnClickListener {
    Context context;
    ...
    public ReView4DBBoxAdapter(Context context, List<T_Detail> detail, List<Boolean> isCheck) {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        if (view == null) {
            view = LayoutInflater.from(context).inflate(R.layout.item_review_list, null);
...
//                ||detail.get(i).activity==Config.DgInActivity||detail.get(i).activity==Config.SimpleInActivity){
        if ("".equals(detail.get(i).FStoreNum) && "".equals(detail.get(i).FBaseNum)){
            viewHolder.tvStorenum.setTextColor(viewHolder.red);
            viewHolder.tvStorenum.setText("箱码数据");
        }else{
            viewHolder.tvStorenum.setTextColor(viewHolder.black);
