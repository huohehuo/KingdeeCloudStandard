...
import com.fangzuo.assist.cloud.Adapter.StoreManSpAdapter;
...
public class SpinnerStoreMan extends RelativeLayout {
    // 返回按钮控件
    private Spinner mSp;
    // 标题Tv
    ...
    private ArrayList<StoreMan> container;
    private StoreManSpAdapter adapter;
    ...
    private String T = "仓管员：";     //3


    public SpinnerStoreMan(Context context, AttributeSet attributeSet) {
...
//        else {
//        StoreManDao employeeDao = daoSession.getStoreManDao();
//        List<StoreMan> employees = employeeDao.loadAll();
...
//    }
    ...
    public StoreManSpAdapter getSpAdapter() {
...
//    }
    ...
    public void setAutoSelection(String saveKeyStr, String string) {
        ...
        for (int j = 0; j < adapter.getCount(); j++) {
            if (((StoreMan) adapter.getItem(j)).FNumber.equals(autoString)
                    || ((StoreMan) adapter.getItem(j)).FName.equals(autoString)) {
                mSp.setSelection(j);
//                autoString = null;
                break;
            }
    ...
    public void setAuto(String saveKeyStr,String autoStr, Org org) {
        ...
        final List<StoreMan> listTemp = getLocData(autoOrg);
...
//                    EventBusUtil.sendEvent(new ClassEvent(EventBusInfoCode.Updata_Error,e.toString()));
    ...
    private List<StoreMan> getLocData(String org) {
        StoreManDao employeeDao = daoSession.getStoreManDao();
        List<StoreMan> employees = employeeDao.queryBuilder().where(StoreManDao.Properties.FOrg.eq(org)
        ).build().list();
        return employees;
    }

    private void dealAuto(List<StoreMan> listData, boolean check) {
        container.clear();
        if (check) {
