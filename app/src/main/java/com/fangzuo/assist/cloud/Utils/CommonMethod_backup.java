...
import com.fangzuo.assist.cloud.Adapter.InStoreTypeSpAdapter;
import com.fangzuo.assist.cloud.Adapter.PDMainSpAdapter;
import com.fangzuo.assist.cloud.Adapter.PayMethodSpAdapter;
import com.fangzuo.assist.cloud.Adapter.PayTypeSpAdapter;
import com.fangzuo.assist.cloud.Adapter.PiciSpAdapter;
...
import com.fangzuo.assist.cloud.Adapter.YuandanSpAdapter;
...
public class CommonMethod_backup {
    ...
    public WaveHouseSpAdapter getWaveHouseAdapter(Storage storage, Spinner sp){
        ...
        WaveHouseDao wavehousedao = daoSession.getWaveHouseDao();
        List<WaveHouse> waveHouses = wavehousedao.queryBuilder().where(WaveHouseDao.Properties.FSPID.eq(storage.FIsOpenWaveHouse)).build().list();
    ...
    public UnitSpAdapter getUnitAdapter(String groupid, Spinner sp){
        ...
        UnitDao unitDao = daoSession.getUnitDao();
        List<Unit> units = unitDao.queryBuilder().where(UnitDao.Properties.FMeasureUnitID.eq(groupid)).build().list();
...
//    }
    ...
    public PiciSpAdapter getPici(Storage storage, String wavehouse, Product product, Spinner spinner){
        ...
        List<InStorageNum> list = inStorageNumDao.queryBuilder().where(
                InStorageNumDao.Properties.FStockID.eq(storage.FItemID),InStorageNumDao.Properties.FItemID.eq(product.FMaterialid),InStorageNumDao.Properties.FQty.gt(0)).build().list();
