...
public class SpinnerAuxSign extends RelativeLayout {
    // 返回按钮控件
    private Spinner mSp;
    // 标题Tv
...
//        dealAuto(listTemp,type,false);
        App.getRService().doIOAction(WebApi.AuxSignFirstCheck, productID, new MySubscribe<CommonResponse>() {
            @Override
...
//        }
    ...
    private void getResultData(String string){
        App.getRService().doIOAction(WebApi.AuxSignSecCheck, string, new MySubscribe<CommonResponse>() {
            @Override
            public void onNext(CommonResponse commonResponse) {
                super.onNext(commonResponse);
