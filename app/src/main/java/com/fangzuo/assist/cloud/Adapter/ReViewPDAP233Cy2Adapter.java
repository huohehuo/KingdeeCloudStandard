...
public class ReViewPDAP233Cy2Adapter extends BaseAdapter implements View.OnClickListener {
    Context context;
    ...
    public ReViewPDAP233Cy2Adapter(Context context, List<T_Detail> detail, List<Boolean> isCheck) {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        if (view == null) {
            view = LayoutInflater.from(context).inflate(R.layout.item_review_pd_p2_33_cy2list, null);
...
//                viewHolder.date.setText("物料名:" + detail.get(i).FProductName);
        ...
        String mdNum=(MathUtil.D2saveInt(MathUtil.toD(detail.get(i).FRealQty)));
        String qty=(MathUtil.D2saveInt(MathUtil.toD(detail.get(i).FQuantity)));
        viewHolder.tvNum.setText("总数量:" + mdNum);
//        viewHolder.tvNum2.setText("码单数量:" + qty);
//        viewHolder.tvNum3.setText("数量差异:" + (MathUtil.toD(mdNum) - MathUtil.toD(qty)));
//        viewHolder.tvNum4.setText("差异比:" + (MathUtil.D2save2(MathUtil.toD(detail.get(i).FRealQty)/ MathUtil.toD(detail.get(i).FQuantity) * 100))+" %");
