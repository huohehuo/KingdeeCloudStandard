...
public class TableAdapter extends BaseAdapter implements View.OnClickListener {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        List<T_main> t_mains = t_mainDao.queryBuilder().where(T_mainDao.Properties.FOrderId.eq(detail.get(i).FOrderId),T_mainDao.Properties.Activity.eq(detail.get(i).activity)).build().list();
        if(t_mains.size()>0){
...
//                viewHolder.price.setText("价格:" + detail.get(i).FTaxUnitPrice);
                ...
                viewHolder.date.setText("下单日期:" + t_mains.get(0).FDate);
            }
